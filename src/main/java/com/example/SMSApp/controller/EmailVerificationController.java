...
//    }
    ...
    @PostMapping("/send-otp")
    public ResponseEntity<?> sendOtp(@RequestBody @Valid OtpRequestDto dto, HttpServletRequest request) {
        String result = emailVerificationService.sendOtp(dto,request);
        return ResponseEntity.ok(result);
    }

    @PostMapping("/verify-otp")
    public ResponseEntity<?> verifyOtp(@RequestBody @Valid OtpVerifyDto otpVerify) {
        boolean verified = emailVerificationService.verifyOtp(otpVerify);
        if (verified) return ResponseEntity.ok("Email verified successfully.");
        return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body("Invalid or expired OTP.");
    }
